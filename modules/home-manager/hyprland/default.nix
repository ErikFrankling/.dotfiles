...

{
  ...
  config = let cfg = config.hyprland;
  in {
    home.pointerCursor = {
      gtk.enable = true;
      # x11.enable = true;
    ...
    home.file = { ".config/hypr/import_env".source = ./import_env; };
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        # resize window 
        (lib.mapAttrsToList
          (key: resize: "$mod ALT,${key},resizeactive,${resize}") resizes) ++
        ...
        workspace =
          [ "w[tv1], gapsout:0, gapsin:0" "f[1], gapsout:0, gapsin:0" ] ++ (map
            (w:
              "${toString w.ID}, monitor:${w.monitor}${
